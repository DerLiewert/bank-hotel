...
class Popup {
    ...
    if (this.options.hashSettings.goHash) {
      ...
      window.addEventListener(
        ...
        function () {
          if (window.location.hash) {
            if (
              !document.querySelector(window.location.hash).hasAttribute('data-popup-no-open-hash')
            ) {
              this._openToHash();
            } else {
              this.hash = ' ';
    ...
    if (bodyLockStatus) {
      ...
      if (this.targetOpen.element) {
        ...
        if (this.options.hashSettings.location) {
          ...
          if (this.targetOpen.element.hasAttribute('data-popup-no-open-hash')) {
            // this.hash = '';
            this._getHash();
          } else {
            this._getHash();
  ...
  _focusCatch(e) {
    ...
    // на случай, если есть элементы или их родители display: hidden
    focusArray = focusArray.filter((element) => {
      return isVisible(element);
    });
    function isVisible(element) {
      const style = window.getComputedStyle(element);

      // Проверяем, скрыт ли сам элемент
      if (style.display === 'none') {
        return false;
      }

      // Проверяем все родительские элементы на display: none
