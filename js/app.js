(() => {
    ...
    class Popup {
        ...
        eventsPopup() {
            ...
            if (this.options.hashSettings.goHash) {
                ...
                window.addEventListener("load", function() {
                    if (window.location.hash) if (!document.querySelector(window.location.hash).hasAttribute("data-popup-no-open-hash")) this._openToHash(); else {
                        this.hash = " ";
                        this._setHash();
                    }
                }.bind(this));
            ...
            if (bodyLockStatus) {
                ...
                if (this.targetOpen.element) {
                    ...
                    if (this.options.hashSettings.location) {
                        if (this.targetOpen.element.hasAttribute("data-popup-no-open-hash")) this._getHash(); else this._getHash();
                        this._setHash();
                    }
                    ...
                    if (!this._reopen) !this.bodyLock ? bodyLock(0) : null; else this._reopen = false;
        ...
        close(selectorValue) {
            ...
            if (!this._reopen) {
                ...
                !this.bodyLock ? bodyUnlock(0) : null;
                this.isOpen = false;
            }
            this._removeHash();
        ...
        _focusCatch(e) {
            ...
            focusArray = focusArray.filter((element => isVisible(element)));
    ...
    class Parallax {
        ...
        destroyEvents() {
            this.elements.forEach((el => {
                el.destroyEvents();
                el.elements.forEach((node => node.style.removeProperty("transform")));
            }));
        }
        setEvents() {
            this.elements.forEach((el => {
    ...
    class ScrollWatcher {
        constructor(props, items) {
            ...
            this.items = items;
            this.scrollWatcherRun(this.items);
        }
        scrollWatcherUpdate(items) {
            this.scrollWatcherOffAll();
            this.items = items;
            this.scrollWatcherRun(this.items);
        }
        scrollWatcherRun(items) {
    ...
    const watchParents = document.querySelectorAll("[data-watch-parent]");
    const watchItems = [];
    watchParents.forEach((watchParent => {
        if (watchParent.dataset.watchParent === "rooms") {
            watchParent.querySelectorAll("[data-watch]").forEach((item => {
                if (!item.closest(".swiper")) watchItems.push(item);
            }));
            return;
        }
        if (watchParent.hasAttribute("data-watch")) watchItems.push(watchParent);
        watchItems.push(...watchParent.querySelectorAll("[data-watch]"));
    }));
    document.addEventListener("DOMContentLoaded", (event => {
        flsModules.watcher = new ScrollWatcher({}, watchItems);
    }));
    ...
    function initSliders() {
        if (document.querySelector(".rooms__slider")) {
            const roomsSlider = new swiper_core_Swiper(".rooms__slider", {
            ...
            const roomsSlideRO = new ResizeObserver((entries => {
                ...
                if (entries[0].contentRect.width > 767.98) roomsNext.style.top = roomsSlideContent.getBoundingClientRect().top - slider.getBoundingClientRect().top + "px"; else roomsNext.style.top = roomsSlideText.getBoundingClientRect().top - slider.getBoundingClientRect().top + "px";
            }));
            ...
            function onMatchMediaChange(e, isSetObserveOnMathes = true) {
                ...
                if (isSetObserveOnMathes) if (e.matches) roomsSlideRO.observe(slider); else {
                    roomsSlideRO.unobserve(slider);
                    slider.querySelector(".rooms__next").style.removeProperty("top");
                } else if (e.matches) {
                    roomsSlideRO.unobserve(slider);
                    slider.querySelector(".rooms__next").style.removeProperty("top");
                } else roomsSlideRO.observe(slider);
            }
        }
    }
    ...
    const times = new Map([ [ "08:00", "08:00 am" ], [ "09:00", "09:00 am" ], [ "10:00", "10:00 am" ], [ "11:00", "11:00 am" ], [ "12:00", "12:00 pm" ], [ "13:00", "01:00 pm" ], [ "14:00", "02:00 pm" ], [ "15:00", "03:00 pm" ], [ "16:00", "04:00 pm" ], [ "17:00", "05:00 pm" ], [ "18:00", "06:00 pm" ], [ "19:00", "07:00 pm" ], [ "20:00", "08:00 pm" ], [ "21:00", "09:00 pm" ], [ "22:00", "10:00 pm" ] ]);
    const bookedDates = [];
    ...
    for (let index = currentMonth.getMonth(); index < currentMonth.getMonth() + 6; index++) {
        ...
        for (;i < lastDate; i++) {
            ...
            if (randomTime.length > 0) bookedDates.push({
                year: temp.getFullYear(),
                month: temp.getMonth(),
                date: randomInteger(index === currentMonth.getMonth() ? currentMonth.getDate() : 1, lastDate),
                time: randomTime
            });
        }
    }
    function randomTimes() {
        ...
        for (let i = 0; i < countDisabledTimes; i++) {
            ...
            while (disabledTimes.includes(time)) time = timesKeys[randomInteger(0, times.size - 1)];
    ...
    function filterBookedDates(year, month) {
        return bookedDates.filter((obj => obj.year == year && obj.month == month && obj?.time.length == times.size)).map((obj => obj.date));
    }
    ...
    class Times {
        constructor(wrapper, id, props = {}) {
            this._times = [ [ "08:00", "08:00 am" ], [ "09:00", "09:00 am" ], [ "10:00", "10:00 am" ], [ "11:00", "11:00 am" ], [ "12:00", "12:00 pm" ], [ "13:00", "01:00 pm" ], [ "14:00", "02:00 pm" ], [ "15:00", "03:00 pm" ], [ "16:00", "04:00 pm" ], [ "17:00", "05:00 pm" ], [ "18:00", "06:00 pm" ], [ "19:00", "07:00 pm" ], [ "20:00", "08:00 pm" ], [ "21:00", "09:00 pm" ], [ "22:00", "10:00 pm" ] ];
            this._id = id;
        ...
        _init() {
            ...
            this.wrapper.addEventListener("click", (e => {
                if (e.target.tagName.toLowerCase() === "input" && e.target.closest(".time__item")) this._config.checkedTime = e.target.getAttribute("value");
            }));
        }
        update(props = {}) {
            this._setConfig(props);
    ...
    class Month {
        constructor(wrapper, id, props = {}) {
            this._weekDays = [ [ "Mon", "Monday" ], [ "Tue", "Tuesday" ], [ "Wed", "Wednesday" ], [ "Thu", "Thursday" ], [ "Fri", "Friday" ], [ "Sat", "Saturday" ], [ "Sun", "Sunday" ] ];
            this._id = id;
            this.wrapper = wrapper;
            this._config = {
                current: {
        ...
        _init(props) {
            ...
            this.wrapper.addEventListener("click", (e => {
                if (e.target.tagName.toLowerCase() == "input" && e.target.closest(".calendar__item--available")) {
                    this._config.checkedDate = e.target.getAttribute("value").split("-")[2];
                    document.dispatchEvent(new CustomEvent("calendarItemClick", {
                        detail: {
        ...
        _setAllInfoForMonth(year = null, month = null) {
            ...
            if (!year && !month) currentMonthInfo = new Date; else currentMonthInfo = new Date(year, month);
        ...
        _createCalendarBody() {
            this.wrapper.innerHTML = "";
            for (let i = 0; i < this._weekDays.length; i++) this._createItem(this._weekDays[i][0], "calendar__item--title");
            for (let i = this._config.current.firstDay - 1; i >= 0; i--) this._createItem("", "calendar__item--empty");
            for (let i = 1; i <= this._config.current.lastDate; i++) if (i < this._config.inactiveToDate) this._createItem(i, "calendar__item--past"); else if (this._config.disableDates.includes(i)) this._createItem(i, "calendar__item--booked"); else {
                const item = this._createItem(i, "calendar__item--available", true);
                if (this._config.checkedDate == i) item.querySelector("input").setAttribute("checked", true);
            }
            for (let i = 0; i < 7 - this._config.current.lastDay; i++) this._createItem("", "calendar__item--empty");
        }
        ...
        _createRadioInput(value) {
            ...
            radioInput.setAttribute("value", `${this._config.current.year}-${("0" + (this._config.current.month + 1)).slice(-2)}-${("0" + value).slice(-2)}`);
            return radioInput;
        }
    ...
    class Calendar {
        ...
        _init() {
            ...
            this._calendarMonth = new Month(this._calendarEl.querySelector(".calendar__body"), this._id, {
                ...
                inactiveToDate: isSameMonth(this._config.currentMonth, this._config.inactiveToDate) ? this._config.inactiveToDate.date : null,
        ...
        _setConfig(props = {}) {
            ...
            this._config.currentMonth = props.currentMonth ? props.currentMonth : {
                year: todayFullDate.getFullYear(),
                month: todayFullDate.getMonth()
            };
            this._config.minMonth = props.minMonth ? props.minMonth : {
                year: todayFullDate.getFullYear(),
                month: todayFullDate.getMonth()
            };
            this._config.maxMonth = props.maxMonth ? props.maxMonth : {
            ...
            this._config.inactiveToDate = props.inactiveToDate ? props.inactiveToDate : {
                year: todayFullDate.getFullYear(),
                month: todayFullDate.getMonth(),
        ...
        _onCalendarNavClick(e) {
            ...
            if (month < 0) {
                ...
                this._config.currentMonth.year = this._config.currentMonth.year + 1;
            } else this._config.currentMonth.month = month;
            this._calendarMonth.update({
                current: this._config.currentMonth,
                disableDates: filterBookedDates(this._config.currentMonth.year, this._config.currentMonth.month),
                inactiveToDate: isSameMonth(this._config.currentMonth, this._config.inactiveToDate) ? this._config.inactiveToDate.date : null,
                checkedDate: isSameMonth(this._config.checkedDate, this._config.currentMonth) ? this._config.checkedDate.date : null
            });
            this._setDateInfoHTML();
        }
        _setDateInfoHTML() {
            this._calendarEl.querySelector(".calendar__full-time").textContent = "";
            this._calendarEl.querySelector(".calendar__full-time").insertAdjacentText("afterbegin", `${this._months[this._config.currentMonth.month]} ${this._config.currentMonth.year}`);
        }
        _createCalendarElement() {
            this.parrent.insertAdjacentHTML("beforeend", `<div class="calendar">\n        <div class="calendar__info">\n          <div class="calendar__full-time"></div>\n          <div class="calendar__navigations">\n            <button class="calendar__nav-button calendar__nav-button--prev">\n              <svg\n                width="10"\n                height="18"\n                viewBox="0 0 10 18"\n                fill="none"\n                xmlns="http://www.w3.org/2000/svg"\n              >\n                <path d="M9 1L1 9L9 17" stroke="#313F38" />\n              </svg>\n            </button>\n            <button class="calendar__nav-button calendar__nav-button--next">\n              <svg\n                width="10"\n                height="18"\n                viewBox="0 0 10 18"\n                fill="none"\n                xmlns="http://www.w3.org/2000/svg"\n              >\n                <path d="M9 1L1 9L9 17" stroke="#313F38" />\n              </svg>\n            </button>\n          </div>\n        </div>\n        <div class="calendar__body"></div>\n      </div>`);
    ...
    class Book {
        ...
        init(props = {}) {
            ...
            this._time = new Times(this.bookEl.querySelector(".time__body"), this._id, {
                ...
                disabledTimes: this._config.bookedDates.find((obj => isSameDate(obj, this._config.checkedDate)))?.time
            });
            this.bookEl.addEventListener("keypress", (e => {
                if (e.target.tagName.toLowerCase() === "input" && e.key === "Enter" && e.target.closest("form")) this.closeBook();
            }));
            ...
            document.dispatchEvent(new CustomEvent("initBook", {
                detail: {
                    target: this.bookEl,
                    parent: this.parrent
                }
    ...
    function initBook(e, from, to, book, config, isLastFocusEl = false) {
        ...
        let bookedDate = null;
        if (from.isActive && from.data.date || to.isActive && !to.data.date && from.data.date) bookedDate = bookedDates.find((obj => isSameDate(obj, from.data))); else if (to.isActive && to.data.date) bookedDate = bookedDates.find((obj => isSameDate(obj, to.data)));
        let minMonth = config.minMonth;
        let inactiveToDate = {
            ...config.inactiveToDate
        };
        let currentMonth = {
            ...config.currentMonth
        };
        ...
        if (to.isActive && from.data.date) {
            ...
            let tempDate;
            if (tempDisabledTimes.length === times.size) tempDate = new Date(from.data.year, from.data.month, from.data.date + 1); else tempDate = new Date(from.data.year, from.data.month, from.data.date);
            inactiveToDate = {
            ...
            if (to.data.date && isSameDate(from.data, to.data)) times.forEach(((value, key) => {
                if (key <= from.data.time && !disabledTimes.includes(key)) disabledTimes.push(key);
            }));
        } else if (from.isActive && from.data.month) currentMonth = {
            year: from.data.year,
    ...
    function confirmSelectDateAndTime(from, to, book) {
        ...
        if (from.isActive) {
            ...
            if (to.data.date && Date.parse(dateToString(from.data) + "T" + from.data.time + ":00") >= Date.parse(dateToString(to.data) + "T" + to.data.time + ":00")) {
                to.data = {
                    year: null,
                    month: null,
                    date: null,
        ...
        } else setItemNewDate(to.el, to.data);
        return true;
        function setItemNewDate(item, data) {
            ...
            item.querySelector(".custom-date span").innerHTML = `${dateToString(data, "/", true)}, ${times.get(data.time)}`;
        }
    }
    function onCalendarItemClick(e, from, to, book, config) {
        ...
        if (to.isActive && from.data.time && isSameDate(checkedDate, from.data)) times.forEach(((value, key) => {
            if (key <= from.data.time && !disabledTimes.includes(key)) disabledTimes.push(key);
        }));
    ...
    document.querySelectorAll("[data-book]").forEach(((bookEl, index) => {
        const book = new Book(bookEl, ++index, {
            bookedDates
        });
        ...
        function onCloseBook(e) {
            ...
            if (from.isActive) from.el.classList.remove("_open-book"); else if (to.isActive) to.el.classList.remove("_open-book");
        ...
        const observer = new ResizeObserver((entries => {
            ...
            if (book.bookEl && activeEl) setBookPosition(book.bookEl, activeEl);
        }));
        function setBookPosition(positionedEl, relativeEl) {
            ...
            if (relativeElRect.left + positionedElRect.width < getWindowWidth()) positionedEl.style.left = relativeElRect.left - parrentRect.left + "px"; else positionedEl.style.left = relativeElRect.left - parrentRect.left - (relativeElRect.left + positionedElRect.width - getWindowWidth()) + "px";
            if (window.scrollY + positionedElRect.top + positionedElRect.height > document.documentElement.scrollHeight - paddingBottom) {
                paddingBottom = window.scrollY + positionedElRect.top + positionedElRect.height - (document.documentElement.scrollHeight - paddingBottom);
    ...
    function bookInPopup() {
        ...
        document.addEventListener("beforePopupOpen", (e => {
            ...
            from = {
                ...
                isActive: false,
                data: bookPopupInfo.get(lastFocusEl) ? bookPopupInfo.get(lastFocusEl).from : {
                    year: null,
                    month: null,
                    date: null,
                    time: null
            ...
            to = {
                ...
                isActive: false,
                data: bookPopupInfo.get(lastFocusEl) ? bookPopupInfo.get(lastFocusEl).to : {
                    year: null,
                    month: null,
                    date: null,
                    time: null
            ...
            if (from.data.date) setInfoToField(from); else from.el.querySelector(".custom-date span").innerHTML = "Check to";
            if (to.data.date) setInfoToField(to); else to.el.querySelector(".custom-date span").innerHTML = "Check out";
        ...
        function setInfoToField(target) {
            target.el.querySelector("input").setAttribute("value", `${dateToString(target.data)}T${target.data.time}`);
            target.el.querySelector(".custom-date span").innerHTML = `${dateToString(target.data, "/", true)}, ${times.get(target.data.time)}`;
        }
        function onInputClick(e) {
            const bookPopup = document.querySelector("#book-popup");
            book = new Book(bookPopup.querySelector(".popup__content"), "popup", {
                bookedDates
            });
    ...
    function facilities() {
        ...
        function setArrowPosition(e) {
            ...
            if (isWatcherCallback && e.detail.entry.target !== startOpenItem.querySelector(".facilities-item__content")) return;
            const openItem = isWatcherCallback ? startOpenItem : e.target.querySelector(".facilities-item.open");
            if (openItem) {
                ...
                const parrentWidth = isWatcherCallback ? facilitiesItemsBlock.clientWidth : e.contentRect.width;
                const marginTop = isWatcherCallback ? parseInt(e.detail.observer.rootMargin.split(" ")[0]) : 0;
                if (parrentWidth > 1023.98) arrow.style.top = content.getBoundingClientRect().top - itemInner.getBoundingClientRect().top + marginTop + "px"; else arrow.style.top = text.getBoundingClientRect().top - itemInner.getBoundingClientRect().top + "px";
            }
        }
